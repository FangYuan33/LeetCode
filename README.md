...
单调栈能在栈内维护单调序列，以 **单调递增栈** 为例，顺序遍历数组入栈，当遇到 **比栈顶元素小的值** 时，该较小的值即为 **当前栈顶元素右侧最近的小值**，若栈顶元素出栈后还有栈内该有元素，则该元素为出栈元素的 **左侧最近的小值**。

一般单调栈的题目会有 **在连续序列中找某个元素左/右侧最近的大/小值** 的特点，我们可以根据找小值还是找大值来确定如下模板中的 while 条件，如果找小值则使用小于号，则 `nums[i] < nums[stack.peek()]`，如果找大值则使用大于号，则 `nums[i] > nums[stack.peek()]`，再根据题意判断是否需要给大于小于号添加上等号 而且 **不要局限** 在只使用一个单调栈解题，有的题目需要维护两个单调栈来求解。
